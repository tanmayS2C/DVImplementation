...
def handle_single_logic_close_ended(single_condition, single_response):
    ...
    if (
        "answers" in single_response
        and left_operand["id"] in single_response["answers"]
    ):
    ...
    if operator == "eq":
        return left_value in right_arr
    else:
        return left_value not in right_arr
...
def handle_single_logic_open_ended(single_condition, single_response):
    ...
    left_operand = (
        str(int(left_operand))
        if (left_operand.isnumeric() and operator != " in ")
        else f" '{left_operand}' "
    )
    right_operand = (
        str(int(right_operand))
        if (right_operand.isnumeric() and operator != " in ")
        else f" '{right_operand}' "
    )
...
def handle_edata(single_condition, single_response):
    ...
    if edata_dv_name in edata_field_arr:
        ...
        res_edata_val = (
            str(int(res_edata_val))
            if res_edata_val.isnumeric()
            else f" '{res_edata_val}' "
        )
        edata_dv_value = (
            str(int(edata_dv_value))
            if edata_dv_value.isnumeric()
            else f" '{edata_dv_value}' "
        )
...
def handle_logic_block(inner_condition, single_response):
    ...
    for single_condition in inner_condition:
        is_openended = (
            True if "itemId" in single_condition["data"]["rightOperand"][0] else False
        )
        ...
        if is_edata:
            ...
            block_result += (
                f" {handle_single_logic_open_ended(single_condition, single_response)} "
            )
        else:
...
def handle_logical(derived_variable, survey_responses, response_collection):
    ...
    for single_response in survey_responses:
        ...
        derivedVar = (
            single_response["derivedVar"] if "derivedVar" in single_response else {}
        )
